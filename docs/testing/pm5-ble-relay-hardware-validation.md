...
| 2026-09-06 (a) | `6.18.42-v8+` | n/a — peripheral only, no PM5 | `pm5-proprietary` | Peripheral-only re-check of `c2m-ooz.3.2`'s fix | `BluezPeripheralServer` exercised standalone on `hci1`: non-bondable/non-discoverable confirmed, `ActiveInstances` 1→0 across `stop()`, 3 consecutive start/stop cycles with 0 failures. Does not cover the iPhone end-to-end (no pairing prompt, reconnect without a wipe) — that is `c2m-ooz.3.3`. Findings §14.3. |
| 2026-09-06 (b) | `6.18.42-v8+` | `8200-000409-217.067` (HW `907`) | `pm5-proprietary` | Root-caused `c2m-ooz.3.1` | Confirmed the app's 8-byte `ce060034` write is rejected by the PM5 (ATT `0x0D`, length 1 declared) and never took effect, which was the whole ~1 s gap — relay's own forward path measures 1.2 ms mean / 4.2 ms max. Fixed by truncating over-length writes to the declared length. Also found and fixed: SIGINT/SIGTERM were not shutting the relay down, so the documented stop procedure silently left the PM5 connected. Findings §14.5. End-to-end re-verification with the app is `c2m-ooz.3.3`. |
